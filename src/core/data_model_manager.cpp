...
DataStructure* DataModelManager::getStructure(const std::string& structureId)
{
    ...
    // Fallback to current structure if ID matches or is empty
    ...
    const std::string& name)
{
    ...
    if (type == "Array") {
        ...
        std::vector<int> data;
        // Sort nodes by their position (assuming node IDs have ordering info)
...
void DataModelManager::selectStructure(const std::string& structureId)
{
    ...
    if (it != metadata.end()) {
        ...
        // Update currentStructure for legacy support
 ...
 // Note: This is a simplification; in a real implementation,
            // you'd need to clone or reference the structure properly
...
DataStructure* DataModelManager::getSelectedStructure()
{
 ...
 if (it != structures.end()) {
   return it->second.get();
    ...
    // Fallback to current structure for legacy support
    return currentStructure.get();
}
...
// Load session from disk and restore all structures
...
// Get all structures as JSON objects for session saving
...
// Restore structures from JSON objects
...
creationType = (structObj["creationType"].toString() == "interactive") ?
 ...
 restoredIds.push_back(id);
    ...
    // Update next ID counter to avoid conflicts
